...
type RouteParams = { params: Promise<{ issueId: string; workLogId: string }> };
...
async function findIssueAndWorkLog(
  ...
  const issue = await findIssueByIdOrKey(issueIdOrKey, { userId });
...
// GET /api/issues/[issueId]/work-logs/[workLogId] - Get a specific work log
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("[WORK_LOG_GET]", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}

// PATCH /api/issues/[issueId]/work-logs/[workLogId] - Update a work log
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const parseResult = UpdateWorkLogSchema.safeParse(body);
...
// DELETE /api/issues/[issueId]/work-logs/[workLogId] - Delete a work log
