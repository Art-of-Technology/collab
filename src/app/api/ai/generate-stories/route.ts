...
export async function POST(request: NextRequest) {
  try {
    ...
    const { epicContext, userEmail } = await request.json();

    if (!epicContext) {
      return NextResponse.json(
        { error: "Epic context is required" },
        { status: 400 }
      );
    }

    // Create the full prompt for story generation from epic
    const systemPrompt = AI_PROMPTS.STORIES_FROM_EPIC(userEmail || session.user.email || "user@example.com");
    const userMessages = [
    ...
    const aiResponse = await generateWithAI(
      systemPrompt,
      userMessages,
      AI_TOKEN_LIMITS.STORIES || 4000,
      AI_CONFIG.MAX_RETRIES || 3
    );

    // Parse the AI response
    const parsedData = await _parseAIResponse(aiResponse, "STORIES generation");
