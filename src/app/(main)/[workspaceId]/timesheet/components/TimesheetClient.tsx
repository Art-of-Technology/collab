...
import type { TimesheetEntry, TimesheetSummary } from "@/app/api/activities/timesheet/route";
...
export function TimesheetClient({ workspaceId, userId }: TimesheetClientProps) {
  ...
  const filteredEntries = timesheetData?.entries.filter((entry) => {
          ...
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center mt-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Select 
                ...
                onValueChange={handleBoardFilter}
              >
                ...
                <Card className="p-8 text-center">
                  ...
                  {timesheetData.entries.length > 0 && filteredEntries.length === 0 && (
                    <Button 
                      variant="outline" 
                      className="mt-4"
                      onClick={() => setLocalFilters({ activityType: 'all', status: 'all' })}
                    >
                      Clear Filters
                    </Button>
                  )}
