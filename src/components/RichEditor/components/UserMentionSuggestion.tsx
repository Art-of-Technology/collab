...
export function UserMentionSuggestion({ 
  query, 
  onSelect, 
  onEscape, 
  workspaceId 
  ...
  return (
    <div 
      ref={containerRef}
      className="bg-popover border rounded-md shadow-md p-1 min-w-[200px] max-h-[200px] overflow-y-auto"
    >
      {users.map((user, index) => (
        <button
          key={user.id}
          className={`w-full flex items-center gap-2 p-2 text-left rounded-sm hover:bg-accent hover:text-accent-foreground transition-colors ${
