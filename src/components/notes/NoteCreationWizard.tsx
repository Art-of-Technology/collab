    ...
    <div className="h-full flex flex-col p-4">
      ...
      {step !== "type" && (
        <div className="flex-none flex items-center gap-2 mb-4">
          <Button
            ...
            size="sm"
            onClick={handleBack}
            className="h-6 px-2 text-collab-500 hover:text-collab-50 group"
          >
        ...
        <div className="flex-1 flex flex-col space-y-4 overflow-auto">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-[13px] font-medium text-collab-50 group-hover:text-white truncate">
                          {config.label}
                        </h3>
                        <p className="text-[11px] text-collab-500/60 truncate">
                          {config.description}
                        </p>
                      </div>
                    </button>
          ...
          <div className="pt-1">
            <Button
              ...
              size="sm"
              onClick={onCancel}
              className="h-6 px-0 text-collab-500 hover:text-collab-50 no-underline hover:no-underline"
            >
              Cancel
            </Button>
          </div>
      ...
      {step === "scope" && selectedType && (
        <div className="flex-1 flex flex-col space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg bg-collab-900 border border-collab-700">
            {(() => {
              ...
              return (
                <>
                  ...
                  <div>
                    <p className="text-[13px] font-medium text-collab-50">{config.label}</p>
                    <p className="text-[11px] text-collab-500">Now choose who can access this content</p>
                  </div>
                </>
              );
            })()}
        ...
        <div className="flex-1 flex flex-col space-y-3">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-9 h-8 text-[13px] bg-collab-900 border-collab-700 text-collab-50 placeholder:text-collab-500/60 focus:border-collab-600"
            />
          </div>

          {/* Project List */}
          <div className="flex-1 min-h-0 rounded-lg border border-collab-700 overflow-hidden flex flex-col">
            {isLoadingProjects ? (
              <div className="flex-1 flex items-center justify-center">
                <div className="h-4 w-4 border-2 border-collab-600 border-t-transparent rounded-full animate-spin" />
              </div>
            ) : filteredProjects.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center">
                <FolderKanban className="h-6 w-6 text-collab-500/50 mb-2" />
                <p className="text-[12px] text-collab-500">
                ...
                <div className="divide-y divide-collab-700">
                  {filteredProjects.map((project) => (
                    ...
                    >
                      ...
                      {/* Project name */}
                      <h3 className="text-[13px] font-medium text-collab-50 group-hover:text-white truncate flex-1">
                        {project.name}
                      </h3>
                    </button>
                  ))}
