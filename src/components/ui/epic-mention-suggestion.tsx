...
interface EpicMentionSuggestionProps {
  query: string;
...
export const EpicMentionSuggestion = forwardRef<HTMLDivElement, EpicMentionSuggestionProps>(
  ({ query, onSelect, workspaceId, onEscape }, ref) => {
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (e.key === "ArrowDown") {
          ...
          setSelectedIndex((prev) => {
            ...
            return prev < epics.length - 1 ? prev + 1 : prev;
          });
          ...
          setSelectedIndex((prev) => {
            ...
            if (prev === -1) return epics.length - 1;
            // Otherwise move to previous item
            return prev > 0 ? prev - 1 : prev;
          });
        } else if (e.key === "Enter" && selectedIndex >= 0 && epics[selectedIndex]) {
          e.preventDefault();
          e.stopPropagation();
    ...
    }, [epics, epics.length, selectedIndex, onSelect, onEscape]);

    // Search epics when query changes
    useEffect(() => {
      const fetchEpics = async () => {
        setLoading(true);
        try {
          // Fetch epics with the query (empty query will return all workspace epics)
          const params = new URLSearchParams({ q: query || '' });
          if (workspaceId) {
            params.append('workspace', workspaceId);
          }
          
          const response = await fetch(`/api/epics/search?${params}`);
          if (response.ok) {
            const searchedEpics = await response.json();
            setEpics(searchedEpics);
            // Reset selected index when new results come in
            setSelectedIndex(-1);
            setIsKeyboardNavigation(false);
          } else {
            console.error('Failed to search epics');
            setEpics([]);
          }
        } catch (error) {
    ...
    useEffect(() => {
      if (commandRef.current && epics.length > 0 && isKeyboardNavigation) {
        const selectedElement = commandRef.current.querySelector(`[data-index="${selectedIndex}"]`);
        if (selectedElement) {
          selectedElement.scrollIntoView({ block: 'nearest' });
        }
      }
    }, [selectedIndex, epics.length, isKeyboardNavigation]);
          ...
          <CommandList className="max-h-[200px] overflow-y-auto">
            {loading ? (
              ...
              <div className="py-6 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-2">No epics found</p>
              </div>
            ) : (
              <CommandGroup>
                  ...
                  >
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <div className="flex flex-col gap-1 flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          {epic.status && (
                            <Badge className={cn("text-xs", getStatusColor(epic.status))}>
                              {epic.status}
                            </Badge>
                          )}
                          <Badge className={cn("text-xs", getPriorityColor(epic.priority))}>
                            {epic.priority}
                          </Badge>
                          {epic.taskBoard && (
...
EpicMentionSuggestion.displayName = "EpicMentionSuggestion"; 