...
export function InlineIssueCreator({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (relationType !== 'child' && parentIssueKey && newIssueKey) {
        await fetch(`/api/workspaces/${workspaceId}/issues/${parentIssueKey}/relations`, {
          method: 'POST',
  ...
  const handleLinkSelected = async () => {
    if (selectedIssues.length === 0 || !onLinkExisting) return;
          ...
          <div className="space-y-1">
            ...
            {showDescription ? (
              ...
              <button
                ...
                className="text-xs text-[#6e7681] hover:text-[#8b949e] transition-colors py-1"
              >
                Add description...
              </button>
            )}
