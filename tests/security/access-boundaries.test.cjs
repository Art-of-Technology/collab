...
test('post sibling readers and mutators enforce access before disclosure or writes', async (t) => {
  ...
  const model = (getRows, kind) => ({
    ...
    delete: async spec => {
      ...
      writes.push([kind, 'delete', rows[index].id]); return rows.splice(index, 1)[0];
    },
  });
  const db = {
  ...
  await t.test('recursive deletion cannot follow a legacy cross-post parent link', async () => {
    for (const invoke of [() => commentActions.deleteComment('c-own'), () => request(commentRoute.DELETE, 'own', null, 'DELETE')]) {
      reset(); comments.find(c => c.id === 'c-foreign').parentId = 'c-own';
      await invoke(); assert.ok(comments.some(c => c.id === 'c-foreign'));
      assert.equal(writes.some(row => row[2] === 'c-foreign'), false);
    }
  });
});
